...
def create_new_words():
    ...
    canvas.itemconfigure(word_text,text=current_card["French"])
    window.after(3000,lambda: flip_card(current_card["English"]))

# ---------------------------- FLIP CARD TO ENGLISH WORD ------------------------------- #
def flip_card(current_word):
    canvas.itemconfig(canvas_image,image=card_back_img)
    canvas.itemconfig(title_text,text="English")
    canvas.itemconfigure(word_text,text=current_word)
